...
Figura::Figura() {
    modo = 4; // 1 -> puntos, 2 -> alambre, 3 -> solido, 4 -> ajedrez
}
...
void Figura::dibujar() {
    if(modo==1) dibujar_puntos();
    if(modo==2) dibujar_alambre();
    if(modo==3) dibujar_solido();
    if(modo==4) dibujar_ajedrez();
}
