...
vector<_vertex3f> v;
vector<_vertex3i> t;
vector<vector<unsigned int> > vecinos;
vector<bool> resaltado;
...
void click_raton(int boton, int estado, int x, int y) {
    switch(boton) {
        default:
            int resultado = pick(x,y);
            if(resultado>=0) {
                resaltado[resultado]=true;
                for(unsigned int i=0; i<vecinos[resultado].size(); i++) {
                    resaltado[vecinos[resultado][i]]=true;
                }
            }
            break;
    }
...
void initialize(void)
{
    ...
    calcular_vecinos(t,vecinos);
    vector<bool> aux(t.size(),false);
    resaltado = aux;
}
...
int main(int argc, char **argv)
{
    ...
    // al bucle de eventos)
    glutCreateWindow("Examen Feb. 2014 - Ejercicio 2");
